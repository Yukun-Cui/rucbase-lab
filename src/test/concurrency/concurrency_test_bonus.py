...
CHECK_METHOD = ["diff_match", "diff_match", "diff_match", "diff_match"]
...
def run():
    ...
    for test_case in TESTS:
            ...
            for key, value in ansDict.items():
                if value != 0:
                    ...
                    comment_str += (
                        "In "
                        + test_case
                        + ", your answer mismatches standard answer.\n"
                    )
                    break
            if match:
                score += 5
        ...
        # close server
        os.system(
            "ps -ef | grep rmdb | grep -v grep | awk '{print $2}' | xargs kill -9"
        )
        print("finish kill")
        # delete database
        # os.system("rm -rf ./" + database_name)
    ...
    if len(FAILED_TESTS) != 0:
        print(
            "\033[0;31;40mConcurrency Bonus Test Final Score:" + str(score) + "\033[0m"
        )
